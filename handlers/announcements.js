...
async function replaceRecipients(announcementId, audienceType, employeeIds) {
  assertDb(await db().from('hadas_announcement_recipients').delete().eq('announcement_id', announcementId), 'לא ניתן לעדכן את מקבלות ההודעה');
  if (audienceType !== 'employees') return;
  const ids = cleanEmployeeIds(employeeIds);
  if (!ids.length) throw httpError(400, 'יש לבחור לפחות עובדת אחת');
  const active = assertDb(await db().from('hadas_employees').select('id').in('id', ids).eq('active', true), 'לא ניתן לבדוק עובדות') || [];
  if (active.length !== ids.length) throw httpError(409, 'אחת העובדות שנבחרו אינה פעילה');
  assertDb(await db().from('hadas_announcement_recipients').insert(ids.map((employeeId) => ({ announcement_id: announcementId, employee_id: employeeId }))), 'לא ניתן לשמור את מקבלות ההודעה');
}
...
module.exports = async function handler(req, res) {
  try {
    ...
    if (!canCreateContent(caller)) throw httpError(403, 'רק מנהלת, אחראית שיבוץ או גננת יכולות לפרסם הודעה');
