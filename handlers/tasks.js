...
async function targetEmployees(targetType, targetId, employeeIds = []) {
  ...
  const employees = assertDb(await query, 'לא ניתן למצוא מקבלות משימה') || [];
  if (targetType === 'employees' && employees.length !== cleanEmployeeIds(employeeIds).length) throw httpError(409, 'אחת העובדות שנבחרו אינה פעילה');
  return employees;
}
...
module.exports = async function handler(req, res) {
  try {
    ...
    if (!canCreateContent(caller)) throw httpError(403, 'רק מנהלת, אחראית שיבוץ או גננת יכולות ליצור משימה');
