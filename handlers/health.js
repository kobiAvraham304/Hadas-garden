...
module.exports = async function handler(req, res) {
  ...
  if (!missing.length) {
    try {
      ...
      checks.schema = meta.error
        ? { ok:false, version:null, error:'יש להריץ פעם אחת את קובץ העדכון update-v0.31.0.sql' }
        : { ok:Boolean(meta.data && meta.data.schema_version === '0.31.0'), version:meta.data?.schema_version || null };
  ...
  return send(res, ready ? 200 : 503, { ok:ready, version, databaseVersion:'0.31.0', checks });
};