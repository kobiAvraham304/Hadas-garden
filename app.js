...
function setBusy(button, busy, text = 'שומרת…') {
  if (!button) return;
  if (busy) {
...
async function init() {
  ...
  try {
    ...
    $('#loginVersion').textContent = `גרסה ${state.config.version || '0.7.0'}`;
...
function bindEvents() {
  ...
  $('#settingsPresetBtn').addEventListener('click', applyDefaultStandards);
...
function renderScheduleLoading() {
  ...
  $('#schedulePublicationState').innerHTML = '<div class="publication-banner"><div><strong>טוענת את השבוע…</strong><small>השיבוץ יופיע מיד.</small></div><span class="loading-dot"></span></div>';
  $('#scheduleWarnings').innerHTML = '';
  $('#scheduleExport').className = `schedule-wrap mode-${state.scheduleMode}`;
  $('#scheduleExport').innerHTML = '<div class="schedule-loading"><span></span><span></span><span></span><p>טוענת שיבוצים…</p></div>';
...
async function setWeek(date) {
  ...
  const key = scheduleCacheKey(target);
  state.weekStart = target;
  const cached = state.weekCache.get(key);
  if (cached) {
    applySchedulePayload(cached.payload);
    renderSchedule();
    fetchScheduleWeek(target, { force: true, apply: true }).then(prefetchAdjacentWeeks).catch(() => {});
    return;
  }
...
function subscribeRealtime() {
  ...
  clearInterval(state.pollTimer); state.pollTimer = setInterval(() => { if (!document.hidden) refreshAll(); }, 90000);
}
...
function renderDashboard() {
  ...
  const quickActions = isManager()
    ? `<section class="quick-actions"><button data-dashboard-tab="schedule" class="quick-blue"><span>▦</span><strong>בניית שיבוץ</strong><small>הוספה, בדיקה ופרסום</small></button><button data-dashboard-tab="employees" class="quick-lilac"><span>♙</span><strong>ניהול עובדות</strong><small>שעות, כיתה ואילוצים</small></button><button data-dashboard-tab="requests" class="quick-coral"><span>↔</span><strong>בקשות והחלפות</strong><small>${pending} ממתינות לטיפול</small></button><button data-dashboard-tab="attendance" class="quick-sun"><span>✓</span><strong>נוכחות היום</strong><small>עדכון ביצוע בפועל</small></button></section>`
    : `<section class="quick-actions"><button data-dashboard-tab="schedule" class="quick-blue"><span>▦</span><strong>השיבוץ השבועי</strong><small>כל הכיתות וכל העובדות</small></button><button data-dashboard-tab="requests" class="quick-coral"><span>↔</span><strong>בקשה חדשה</strong><small>חופשה, מחלה או החלפה</small></button><button data-dashboard-tab="tasks" class="quick-lilac"><span>☑</span><strong>המשימות שלי</strong><small>${dueTasks} פתוחות</small></button><button data-dashboard-tab="announcements" class="quick-sun"><span>◉</span><strong>הודעות</strong><small>${unread} טרם נקראו</small></button></section>`;
...
function renderMobileWeekClass(classItem, date) {
  ...
  return `<section class="mobile-week-class"><div class="mobile-week-class-head"><div><h4>${escapeHtml(classItem.name)}</h4><small>${coverage.count} משובצות · ${coverage.closing} בסגירה</small></div><span class="status-chip ${coverage.ok ? 'ok' : 'error'}">${coverage.ok ? 'תקין' : 'דורש טיפול'}</span></div><div class="mobile-week-class-shifts">${rows.length ? rows.map((shift) => shiftCardHtml(shift, true)).join('') : '<div class="empty-state compact">אין שיבוצים בכיתה</div>'}</div>${isManager() ? `<button class="mobile-add-shift" data-action="add" data-date="${iso}" data-class="${classItem.id}">＋ הוספת עובדת לכיתה</button>` : ''}</section>`;
}
function renderMobileWeekDay(date, index) {
  ...
  const open = index === state.scheduleDay || (!currentWeekDates().some((item) => dateISO(item) === dateISO(new Date())) && index === 0);
  return `<details class="mobile-week-day ${issues ? 'has-issue' : ''}" ${open ? 'open' : ''}><summary><span class="mobile-week-day-name"><strong>${DAY_NAMES[date.getDay()]}</strong><small>${formatDate(date, { day: '2-digit', month: '2-digit' })}</small></span><span class="mobile-week-day-stats"><b>${new Set(dayRows.map((shift) => shift.employee_id)).size}</b><small>עובדות</small></span><span class="mobile-week-day-status ${issues ? 'issue' : 'ok'}">${issues ? `${issues} כיתות לבדיקה` : 'כל הכיתות תקינות'}</span><span class="mobile-week-chevron">⌄</span></summary><div class="mobile-week-day-body">${state.classes.filter((item) => item.active).map((item) => renderMobileWeekClass(item, date)).join('')}</div></details>`;
}
function renderScheduleWeek() {
  ...
  const mobile = `<div class="schedule-mobile-week"><div class="mobile-week-intro"><span>שבוע מלא בלי גלילה לצדדים</span><small>לחצי על יום כדי לפתוח את כל הכיתות והשיבוצים.</small></div>${dates.map(renderMobileWeekDay).join('')}</div>`;
...
function renderEmployees() {
  ...
  $('#employeesList').innerHTML = filtered.length ? filtered.map((employee) => { const type = employeeType(employee); return `<article class="employee-card employee-type-${type} ${employee.active ? '' : 'inactive'}"><div class="employee-card-accent"></div><span class="employee-card-status status-chip ${employee.active ? 'ok' : 'error'}">${employee.active ? 'פעילה' : 'לא פעילה'}</span><div class="employee-card-head"><span class="employee-avatar">${escapeHtml(initials(employee.full_name))}</span><div><h3>${escapeHtml(employee.full_name)}</h3><p>${escapeHtml(employee.job_title)} · ${escapeHtml(classById(employee.primary_class_id)?.name || 'ללא כיתה קבועה')}</p></div></div><div class="employee-role-strip"><span>${type === 'teacher' ? '✿ גננת' : type === 'assistant' ? '☀ סייעת' : type === 'therapy' ? '★ פרא־רפואי' : type === 'management' ? '♙ ניהול' : '• צוות'}</span>${employee.can_lead ? '<span>יכולה להוביל</span>' : ''}</div><div class="employee-card-meta"><div><small>טלפון</small><strong>${escapeHtml(employee.phone || '—')}</strong></div><div><small>הרשאה</small><strong>${escapeHtml(ROLE_LABELS[employee.role] || 'עובדת')}</strong></div><div><small>שעות שבועיות</small><strong>${employee.weekly_hours ?? 'לא הוגדר'}</strong></div><div><small>שעות רגילות</small><strong>${timeHtml(employee.default_start, employee.default_end)}</strong></div></div><div class="card-actions"><button class="secondary-btn" data-action="edit" data-id="${employee.id}"><span>✎</span> עריכת כרטיס</button><button class="ghost-btn" data-action="reset" data-id="${employee.id}">איפוס סיסמה</button><button class="${employee.active ? 'danger-btn' : 'primary-btn'}" data-action="toggle" data-id="${employee.id}">${employee.active ? 'השבתה' : 'הפעלה'}</button></div></article>`; }).join('') : '<div class="empty-state">לא נמצאו עובדות לפי הסינון.</div>';
}
...
function renderSettingsPreview() {
  ...
  const invalid = timeToMinutes(closing) <= timeToMinutes(opening) || required < 1 || closingRequired < 1 || closingRequired > required || windowMinutes < 15;
  $('#settingsPreview').innerHTML = invalid
    ? '<div class="notice error">יש לתקן את הערכים המסומנים לפני השמירה.</div>'
    : `<div class="standards-preview-title"><span>תצוגה מקדימה</span><strong>${opening}–${closing}</strong></div><div class="standards-timeline"><div class="timeline-rule regular"><span>${opening}</span><strong>${required} נשות צוות בכל כיתה</strong><small>נבדק כל ${slot} דקות · כולל גננת או מובילה</small></div><div class="timeline-arrow">←</div><div class="timeline-rule closing"><span>${closingStart}</span><strong>${closingRequired} נשות צוות בסגירה</strong><small>עד שעת הסגירה ${closing}</small></div></div>`;
}
function applyDefaultStandards() {
  const form = $('#settingsForm');
  form.elements.opening_time.value = '07:30';
  form.elements.closing_time.value = '15:30';
  form.elements.required_staff.value = '4';
  form.elements.closing_required_staff.value = '3';
  form.elements.closing_window_minutes.value = '30';
