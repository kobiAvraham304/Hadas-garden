...
function autoReviewSuggestionHtml(item){
  ...
  return `<article class="auto-review-suggestion" data-auto-review-id="${escapeHtml(item.id||'')}"><div class="auto-review-suggestion-copy"><span>הצעת השלמה לחלק מהיום</span><strong>${escapeHtml(employee?.full_name||'עובד/ת')} · ${escapeHtml(classItem?.name||'כיתה')}</strong><small>${item.shift_date?formatDate(item.shift_date):''} · ${timeHtml(item.start_time,item.end_time)}</small><p>${escapeHtml(item.explanation||'המערכת מצאה אפשרות בטוחה להשלמת החוסר, אך לא הוסיפה אותה אוטומטית כי זו משמרת חלקית.')}</p>${reasons?`<em>למה: ${escapeHtml(reasons)}</em>`:''}</div><button type="button" class="secondary-btn auto-review-add" data-auto-review-add="${escapeHtml(item.id||'')}">＋ הוסף לשיבוץ</button></article>`;
}
function autoReviewSuggestionRow(item){
  return {shift_date:item.shift_date,class_id:item.class_id,employee_id:item.employee_id,start_time:item.start_time,end_time:item.end_time,shift_role:item.shift_role||'staff',public_note:'אושר מתוך המלצות השיבוץ האוטומטי'};
}
async function approveAutoReviewSuggestion(item,button){
  if(!item)return;
  const row=autoReviewSuggestionRow(item);
  const duplicate=state.autoScheduleManualGenerated.some((existing)=>existing.shift_date===row.shift_date&&existing.class_id===row.class_id&&existing.employee_id===row.employee_id&&trimTime(existing.start_time)===trimTime(row.start_time)&&trimTime(existing.end_time)===trimTime(row.end_time));
  ...
  state.autoScheduleManualGenerated.push(row);
  setBusy(button,true,'מוסיף…');
  try{
    await revalidateAutomaticPreview();
    showToast('ההמלצה נוספה לשיבוץ המתוכנן ונבדקה מחדש','success');
  }catch(error){
