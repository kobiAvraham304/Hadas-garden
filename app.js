...
function renderAutomaticSchedulePreview(preview) {
  ...
  const metrics = preview.metrics || {}; const errors = preview.validation?.errors || []; const warnings=preview.validation?.warnings||[];
  ...
  const warningHtml=warnings.length?`<details class="auto-warning-list"><summary><strong>${warnings.length} המלצות לבדיקה</strong><span>אינן חוסמות את השמירה</span><i>⌄</i></summary><div>${warnings.map((item)=>`<p><strong>${escapeHtml(item.message||'מומלץ לבדוק את השיבוץ')}</strong><small>${[item.date?formatDate(item.date):'',classById(item.class_id)?.name||'',item.start_time&&item.end_time?`${trimTime(item.start_time)}–${trimTime(item.end_time)}`:''].filter(Boolean).join(' · ')}</small></p>`).join('')}</div></details>`:'';
