...
async function finishManagerPreapprovedSwap(request) {
  ...
  const approved = assertDb(await db().from('hadas_requests').update({
    status: 'approved',
    decided_by: request.created_by,
    decided_at: new Date().toISOString(),
  }).eq('id', request.id).select('*').single(), 'העובד אישר אך לא ניתן היה להשלים את האישור המוקדם');
  assertDb(await db().rpc('hadas_apply_approved_request', {
    p_request_id:approved.id,
    p_actor_id:approved.created_by,
  }), 'ההחלפה אושרה אך לא ניתן היה להזרים אותה לשיבוץ');
  const updated = assertDb(await db().from('hadas_requests').select('*').eq('id', approved.id).single(), 'לא ניתן לרענן את הבקשה לאחר ההזרמה');
  ...
  await notifyManagers({
    type: 'request', title: 'החלפה שאושרה מראש הוזרמה לשיבוץ',
    message: 'העובד שנבחר אישר את ההחלפה. השיבוץ עודכן אוטומטית בטיוטה.',
...
module.exports = async function requestsV030(req, res) {
  try {
    ...
    if (req.method === 'POST' && ['delete_pending','request_cancellation','approve_cancellation','reject_cancellation'].includes(action)) {
      const caller = await requireSession(req);
      if (!body.id) throw httpError(400,'חסר מזהה בקשה');
      if (['approve_cancellation','reject_cancellation'].includes(action) && !isManager(caller)) throw httpError(403,'אין הרשאה לאישור ביטול');
      const result = assertDb(await db().rpc('hadas_leave_action_v0361', {
        p_request_id:String(body.id),p_actor_id:caller.employee.id,p_action:action,
      }), 'לא ניתן לעדכן את בקשת החופשה');
      const jobs = [audit(caller.employee.id,action,'request',body.id),emitEvent('requests')];
      if (action === 'request_cancellation') jobs.push(notifyManagers({type:'request',title:'בקשת ביטול חופשה ממתינה לאישור',message:`${caller.employee.full_name} מבקש/ת לבטל חופשה שאושרה. החופשה בתוקף עד אישור הביטול.`,entityType:'request',entityId:body.id,actionRequired:true}));
      if (['approve_cancellation','reject_cancellation'].includes(action)) {
        jobs.push(notifyEmployees([result.requester_id],{type:'request',title:action==='approve_cancellation'?'ביטול החופשה אושר':'ביטול החופשה לא אושר',message:action==='approve_cancellation'?'החופשה בוטלה והוסרה מהשיבוץ ומלוח השנה.':'החופשה המאושרת נשארת בתוקף.',entityType:'request',entityId:body.id,actionRequired:false}));
        jobs.push(db().from('hadas_notifications').update({action_required:false}).eq('entity_type','request').eq('entity_id',String(body.id)));
      }
      if (result.deleted) jobs.push(emitEvent('shifts'),emitEvent('calendar'));
      await Promise.all(jobs);
      const request = result.deleted ? null : assertDb(await db().from('hadas_requests').select('*').eq('id',body.id).single(),'לא ניתן לרענן בקשה');
      return send(res,200,{ok:true,request,deleted_id:result.deleted?result.id:null});
    }
    ...
    if (req.method === 'POST' && action === 'create') {
      ...
      if (!managerSubmitted) {
        if (created.status !== 'pending') throw httpError(409, 'בקשה שעובד מגיש בעצמו אינה יכולה להיות מאושרת מראש');
        payload.request = created;
        payload.manager_preapproved = false;
        return replayResponse(captured, res, payload);
      }
      const managerPreapproved = truthy(body.pre_approved) || truthy(body.apply_now);
