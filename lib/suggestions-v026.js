...
function previewCandidates(ranking, virtualShifts, mode) {
  if (!virtualShifts || mode !== 'add') return { candidates: ranking.candidates, rejected: ranking.rejected };
  const transferCandidates = ranking.candidates.filter((item) => item.candidate_type === 'transfer');
  const candidates = ranking.candidates.filter((item) => item.candidate_type !== 'transfer');
  const rejected = [
    ...ranking.rejected,
    ...transferCandidates.map((item) => ({
      employee_id: item.employee_id,
      full_name: item.full_name,
      reason: 'כבר משובץ בטיוטה המוצעת בשעות האלה',
      code: 'already_scheduled_in_preview',
    })),
  ];
...
module.exports = async function handler(req, res) {
  try {
    ...
    // כך עובד שכבר תפוס בטיוטה לא יוצע כעובד פנוי, וגם מכסות מחושבות מול המצב העתידי.
    ...
    const filtered = previewCandidates(ranking, virtualShifts, mode);
...
module.exports.previewCandidates = previewCandidates;
