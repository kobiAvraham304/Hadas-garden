    ...
    while (true) {
      ...
      for (const employee of availableNeedWorkers) {
        ...
        for (const classItem of activeClasses) {
          ...
          if (!segment) continue;
          // "לפי צורך" אינו נכנס אוטומטית לחלק מהיום. רק אם כל חלון
          // הזמינות שלו נדרש בפועל, אפשר לשבץ אותו אוטומטית. מקטע חלקי
          // נשאר להצעה מפורשת למנהלת בתצוגה המקדימה.
          const dayOpen = timeToMinutes(settings.opening_time || '07:30');
          const dayClose = timeToMinutes(closingTimeForDate(settings, date));
          const coversWholeOperatingDay = segment.start <= dayOpen && segment.end >= dayClose;
          if (!coversWholeOperatingDay) continue;
