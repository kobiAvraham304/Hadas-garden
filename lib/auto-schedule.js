    ...
    while (true) {
      ...
      for (const employee of availableNeedWorkers) {
        ...
        for (const classItem of activeClasses) {
          ...
          const fullWindowNeeded=fullWindowSlots.length>0&&fullWindowSlots.every((slot)=>slot.staffMissing>0||(canLead(employee)&&slot.leaderMissing));
          const fullWindowDuration=availabilityEnd-availabilityStart;
          if(fullWindowNeeded&&fullWindowDuration>0&&fullWindowDuration<=remainingFor(employee)&&!existingSegments.length){
            ...
            segment={start:availabilityStart,end:availabilityEnd,staffUnits,leaderUnits,idleUnits:0,duration:fullWindowDuration,utility:staffUnits*30+leaderUnits*55-fullWindowDuration/180};
          }
