...
function generateAutomaticSchedule(input) {
  ...
  // הצעות חלקיות אינן נשמרות אוטומטית. הן מוצגות כהמלצות לבדיקה
  // וניתנות להוספה מפורשת מהתצוגה המקדימה.
  ...
  for (const gap of coverageGaps) {
    ...
    for (const employee of activeEmployees) {
      ...
      const utility = base.score + (needsLeader ? 75 : 45) - Math.max(0, proposedEnd - proposedStart - (gapEnd - gapStart)) / 60;
      options.push({ employee, start, end, baseScore:base.score, reasons, utility });
    }
    const chosen = stableSortCandidates(options)[0];
    if (!chosen) continue;
