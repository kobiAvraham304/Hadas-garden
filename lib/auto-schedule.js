...
const {
  ...
  closingTimeForDate, calculateWeeklyMinutes, validateWeek,
} = require('./schedule');
...
function employeeBaseScore({ employee, classItem, date, constraints, availability, previousShifts }) {
  ...
  if (constraint?.constraint_type === 'forbidden') return { eligible: false, score: -Infinity, reasons: ['קיים איסור שיבוץ בכיתה'] };
...
function requiredAt(settings, date, minute) {
  ...
  return minute >= closing - closingWindow ? Number(settings.closing_required_staff || 3) : Number(settings.required_staff || 4);
}
...
function generateAutomaticSchedule(input) {
  ...
  const cleanGenerated = generated.map(({ _score, _reasons, ...row }) => row);
  ...
  return {
    ...
    validation, daySummaries, employeeHours, excluded,
