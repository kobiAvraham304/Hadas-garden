...
function employeeAvailability({ employee, date, patterns, requests, settings }) {
  ...
  let startMin = Math.max(timeToMinutes(start), timeToMinutes(open));
  let endMin = Math.min(timeToMinutes(end), timeToMinutes(close));
  for (const request of requests) {
    if (request.requester_id === employee.id && request.request_type === 'late_start'
      && ['approved','applied'].includes(request.status) && request.request_date === date && request.requested_start) {
      startMin = Math.max(startMin, timeToMinutes(request.requested_start));
    }
    if (request.requester_id === employee.id && request.request_type === 'early_finish'
      && ['approved','applied'].includes(request.status) && request.request_date === date && request.requested_end) {
      endMin = Math.min(endMin, timeToMinutes(request.requested_end));
    }
  }
...
function automaticInputSignature(input, dates, mode) {
  const snapshot = {
    dates, mode,
    employees: input.employees || [], classes: input.classes || [], patterns: input.patterns || [],
    constraints: input.constraints || [], requests: input.requests || [], settings: input.settings || {},
    generalDaysOff: input.generalDaysOff || [],
    existingShifts: input.existingShifts || [], previousShifts: input.previousShifts || [],
  };
  return crypto.createHash('sha256').update(JSON.stringify(stableValue(snapshot))).digest('hex').slice(0, 24);
}
function expandPracticalSegment(row, availability, remainingMinutes, blockedRows = [], minimumMinutes = 120) {
  let start = timeToMinutes(row.start_time); let end = timeToMinutes(row.end_time);
  const duration = end - start;
  if (duration <= 0 || duration >= minimumMinutes || remainingMinutes <= 0) return { start, end, expanded: false };
  let lower = timeToMinutes(availability.start); let upper = timeToMinutes(availability.end);
  for (const blocked of blockedRows) {
    const blockedStart = timeToMinutes(blocked.start_time); const blockedEnd = timeToMinutes(blocked.end_time);
    if (blockedEnd <= start) lower = Math.max(lower, blockedEnd);
    else if (blockedStart >= end) upper = Math.min(upper, blockedStart);
  }
  const target = Math.floor(Math.min(minimumMinutes, duration + remainingMinutes, upper - lower) / 15) * 15;
  if (target <= duration) return { start, end, expanded: false };
  const extra = target - duration;
  const centeredStart = start - Math.floor(extra / 30) * 15;
  const nextStart = Math.max(lower, Math.min(centeredStart, upper - target));
  return { start: nextStart, end: nextStart + target, expanded: true };
}
function partialAsNeededIssue({ row, employee, availability, minimumMinutes = 120 }) {
  if (!availability || (shortTime(row.start_time) === shortTime(availability.start) && shortTime(row.end_time) === shortTime(availability.end))) return null;
  const duration = timeToMinutes(row.end_time) - timeToMinutes(row.start_time);
  const practicalMinimum = Math.min(minimumMinutes, timeToMinutes(availability.end) - timeToMinutes(availability.start));
  const short = duration < practicalMinimum;
  return {
    severity: short ? 'error' : 'warning',
    code: short ? 'short_nonfixed_shift' : 'partial_as_needed_shift',
    date: row.shift_date, class_id: row.class_id, employee_id: row.employee_id,
    start_time: shortTime(row.start_time), end_time: shortTime(row.end_time),
    message: short
      ? `${employee.full_name}: משמרת לפי צורך קצרה מדי (${shortTime(row.start_time)}–${shortTime(row.end_time)}) — נדרש תיקון או אישור חריגה`
      : `${employee.full_name}: שובץ/ה לפי צורך רק בשעות ${shortTime(row.start_time)}–${shortTime(row.end_time)}`,
  };
}
...
function generateAutomaticSchedule(input) {
  const {
    ...
    settings = {}, generalDaysOff = [], existingShifts = [], previousShifts = [], mode = 'rebuild', createdBy = null, selectedDates = null,
  } = input;
  ...
  const generalDayOffDates=new Set((generalDaysOff||[]).filter((row)=>row?.is_general_day_off!==false).map((row)=>String(row?.event_date||row?.date||'')).filter(Boolean));
  const dates=weekDates.filter((date)=>selectedSet.has(date)&&!generalDayOffDates.has(date));
  ...
  function wouldExceedClassMax({employeeId,classId,date,start,end,ignoreRow=null}) {
    if(maxDailyStaff==null)return false;
    ...
    const overlapping=allRows.filter((row)=>row!==ignoreRow&&row.shift_date===date&&row.class_id===classId&&timeToMinutes(row.start_time)<endMin&&timeToMinutes(row.end_time)>startMin);
    const boundaries=new Set([startMin,endMin]);
    for(const row of overlapping){
      boundaries.add(Math.max(startMin,timeToMinutes(row.start_time)));
      boundaries.add(Math.min(endMin,timeToMinutes(row.end_time)));
    }
    const points=[...boundaries].sort((a,b)=>a-b);
    for(let index=0;index<points.length-1;index+=1){
      const point=points[index];
      if(points[index+1]<=point)continue;
      const people=new Set(overlapping.filter((row)=>timeToMinutes(row.start_time)<=point&&timeToMinutes(row.end_time)>point).map((row)=>row.employee_id));
      people.add(employeeId);
      if(people.size>maxDailyStaff)return true;
    }
    return false;
  }
  ...
  function addGeneratedShift({ employee, classItem, date, start, end, reasons = [], baseScore = 70, note = null, kind = 'regular' }) {
    ...
    if(wouldExceedClassMax({employeeId:employee.id,classId:classItem.id,date,start,end})){
      excluded.push({date,employee_id:employee.id,reason:`מקסימום התקינה בכיתה הוא ${maxDailyStaff} עובדים`,mandatory:kind==='fixed_work'||kind==='floating_work'});
      return null;
    }
    const shift = {
  ...
  for (const date of dates) {
    ...
    for (const { employee, availability } of floatingWork) {
      ...
      const options = activeClasses.map((classItem) => {
        ...
        if(wouldExceedClassMax({employeeId:employee.id,classId:classItem.id,date,start:availability.start,end:availability.end}))return null;
    ...
    while (true) {
      ...
      for (const employee of availableNeedWorkers) {
        ...
        for (const classItem of activeClasses) {
          ...
          let segment = bestNeedSegment(coverage, availability, canLead(employee), remainingFor(employee), settings, existingSegments);
          if (!segment) continue;
          // עובד/ת "לפי צורך" נכנס/ת אוטומטית רק אם החוסר מכסה את מלוא
          // שעות הזמינות הרגילות שלו/ה באותו יום. אם צריך רק חלק מהחלון,
          // ההשלמה נשארת כהמלצה מפורשת למנהלת.
          const availabilityStart=timeToMinutes(availability.start),availabilityEnd=timeToMinutes(availability.end);
          const fullWindowSlots=coverage.filter((slot)=>slot.start>=availabilityStart&&slot.end<=availabilityEnd);
          const fullWindowDuration=availabilityEnd-availabilityStart;
          const neededMinutes=fullWindowSlots.reduce((sum,slot)=>sum+((slot.staffMissing>0||(canLead(employee)&&slot.leaderMissing))?(slot.end-slot.start):0),0);
          const needRatio=fullWindowDuration>0?neededMinutes/fullWindowDuration:0;
          // אם רוב חלון העבודה הרגיל נדרש (75%+), משבצים את כל החלון ולא
          // מקטע חלקי. חוסר קטן/נקודתי נשאר כהמלצה לאישור ידני.
          const useWholeAvailability=fullWindowSlots.length>0&&needRatio>=0.75&&fullWindowDuration<=remainingFor(employee)&&!existingSegments.length;
          if(useWholeAvailability){
            const staffUnits=fullWindowSlots.filter((slot)=>slot.staffMissing>0).length;
            const leaderUnits=canLead(employee)?fullWindowSlots.filter((slot)=>slot.leaderMissing).length:0;
            const idleUnits=fullWindowSlots.length-staffUnits-Math.min(leaderUnits,fullWindowSlots.length-staffUnits);
            segment={start:availabilityStart,end:availabilityEnd,staffUnits,leaderUnits,idleUnits:Math.max(0,idleUnits),duration:fullWindowDuration,utility:staffUnits*30+leaderUnits*55-Math.max(0,idleUnits)*10-fullWindowDuration/180};
          }
          const coversNormalAvailability = segment.start <= availabilityStart && segment.end >= availabilityEnd;
          if (!coversNormalAvailability) continue;
          if(wouldExceedClassMax({employeeId:employee.id,classId:classItem.id,date,start:minutesToTime(segment.start),end:minutesToTime(segment.end)}))continue;
          let scarcityBonus = 0;
    ...
    while (true) {
      ...
      for (const shift of generated.filter((row) => row.shift_date === date && row._kind === 'fixed_work')) {
        ...
        for (const classItem of activeClasses) {
          ...
          if(wouldExceedClassMax({employeeId:employee.id,classId:classItem.id,date,start:shift.start_time,end:shift.end_time,ignoreRow:shift}))continue;
  ...
  // Turn tiny coverage fragments into practical shifts only after every real
  // deficit in the selected period has had first claim on the weekly limits.
  // This keeps a cosmetic extension on Sunday from starving a true need later
  // in the week.
  for (const row of generated.filter((item) => item._kind === 'as_needed')) {
    ...
    if (!employee || !availability) continue;
    const blocked = employeeDayRows(row.employee_id, row.shift_date).filter((item) => item !== row);
    const expanded = expandPracticalSegment(row, availability, remainingFor(employee), blocked);
    if (!expanded.expanded) continue;
    const expandedStart=minutesToTime(expanded.start),expandedEnd=minutesToTime(expanded.end);
    if(wouldExceedClassMax({employeeId:row.employee_id,classId:row.class_id,date:row.shift_date,start:expandedStart,end:expandedEnd,ignoreRow:row}))continue;
    const previousDuration = timeToMinutes(row.end_time) - timeToMinutes(row.start_time);
    row.start_time = expandedStart; row.end_time = expandedEnd;
    const addedMinutes = expanded.end - expanded.start - previousDuration;
    weeklyMinutes.set(employee.id, (weeklyMinutes.get(employee.id) || 0) + addedMinutes);
    row._reasons.push('המשמרת הורחבה לאורך מעשי בלי ליצור חפיפה או חריגה ממכסת השעות');
  }
  ...
  const partialNonFixed=generated.filter((row)=>row._kind==='as_needed').flatMap((row)=>{
    const employee=activeEmployees.find((item)=>item.id===row.employee_id);
    const availability=employee&&employeeAvailability({employee,date:row.shift_date,patterns,requests,settings});
    const issue=employee&&partialAsNeededIssue({row,employee,availability});return issue?[issue]:[];
  });
  validation.errors.push(...partialNonFixed.filter((item)=>item.severity==='error').map(({severity,...item})=>item));
  validation.warnings.push(...partialNonFixed.filter((item)=>item.severity==='warning').map(({severity,...item})=>item));
  ...
  const coverageGaps = validation.errors.filter((item) => ['understaffed','missing_leader'].includes(item.code)).map((item) => {
    const start=timeToMinutes(item.time||item.start_time);const end=timeToMinutes(item.end_time||item.time||item.start_time);
    const classItem=activeClasses.find((row)=>row.id===item.class_id);
    const roleRequired=item.code==='missing_leader';
    const available=activeEmployees.filter((employee)=>{const availability=employeeAvailability({employee,date:item.date,patterns,requests,settings});return availability&&timeToMinutes(availability.start)<=start&&timeToMinutes(availability.end)>=end;});
    const eligible=available.filter((employee)=>{
      if(roleRequired&&!canLead(employee))return false;
      if(!classEligibility({employee,classItem,date:item.date,constraints}).eligible)return false;
      if(finalRows.some((row)=>row.employee_id===employee.id&&row.shift_date===item.date&&timeToMinutes(row.start_time)<end&&timeToMinutes(row.end_time)>start))return false;
      if(calculateWeeklyMinutes(finalRows,employee.id)+(end-start)>maxMinutes(employee))return false;
      const limit=Number(employee.max_work_days_per_week);const days=new Set(finalRows.filter((row)=>row.employee_id===employee.id).map((row)=>row.shift_date));
      return !Number.isInteger(limit)||limit<1||days.has(item.date)||days.size<limit;
    });
    const reason=!available.length?'no_available_worker':!eligible.length?'no_eligible_worker':'limited_capacity';
    const explanation=reason==='no_available_worker'?'אין עובד/ת שמוגדר/ת כזמין/ה בשעות האלה':reason==='no_eligible_worker'?'יש עובדים זמינים, אך כולם משובצים, חסומים לכיתה או הגיעו למכסה':`נמצאו ${eligible.length} מועמדים פנויים לבדיקה ידנית`;
    return {date:item.date,class_id:item.class_id,start_time:minutesToTime(start),end_time:minutesToTime(end),code:item.code,reason,explanation,candidate_count:eligible.length,candidate_employee_ids:eligible.slice(0,5).map((employee)=>employee.id)};
  });


  // הצעות חלקיות אינן נשמרות אוטומטית. המנוע קודם מחפש פתרון יום
  // מרוכז: עדיף להביא עובד/ת אחד/ת לרצף עבודה הגיוני ואף להעביר בין
  // כיתות, מאשר ליצור כמה הגעות קצרות או להזמין כמה עובדים ללא צורך.
  const suggestionKeys = new Set();
  const orderedGaps=[...coverageGaps].sort((a,b)=>String(a.date).localeCompare(String(b.date))||timeToMinutes(a.start_time)-timeToMinutes(b.start_time)||String(a.class_id).localeCompare(String(b.class_id)));
  for (const gap of orderedGaps) {
    const classItem = activeClasses.find((row) => row.id === gap.class_id);
    if (!classItem) continue;
    const gapStart = timeToMinutes(gap.start_time);
    const gapEnd = timeToMinutes(gap.end_time);
    if (gapEnd <= gapStart) continue;
    const needsLeader = gap.code === 'missing_leader';
    const options = [];
    for (const employee of activeEmployees) {
      const availability = employeeAvailability({ employee, date:gap.date, patterns, requests, settings });
      if (!availability?.asNeeded) continue;
      if (needsLeader && !canLead(employee)) continue;
      if (timeToMinutes(availability.start) > gapStart || timeToMinutes(availability.end) < gapEnd) continue;
      if (!classEligibility({ employee, classItem, date:gap.date, constraints }).eligible) continue;
      const blocked = finalRows.filter((row) => row.employee_id === employee.id && row.shift_date === gap.date);
      if (blocked.some((row) => timeToMinutes(row.start_time) < gapEnd && timeToMinutes(row.end_time) > gapStart)) continue;
      const priorForEmployee=rawReviewSuggestions.filter((item)=>item.shift_date===gap.date&&item.employee_id===employee.id);
      if(priorForEmployee.some((item)=>timeToMinutes(item.gap_start_time||item.start_time)<gapEnd&&timeToMinutes(item.gap_end_time||item.end_time)>gapStart))continue;
      const weeklyUsed = calculateWeeklyMinutes(finalRows, employee.id);
      const remaining = Math.max(0, maxMinutes(employee) - weeklyUsed);
      if (remaining < gapEnd - gapStart) continue;
      const limit = Number(employee.max_work_days_per_week);
      const days = new Set(finalRows.filter((row) => row.employee_id === employee.id).map((row) => row.shift_date));
      if (Number.isInteger(limit) && limit > 0 && !days.has(gap.date) && days.size >= limit) continue;

      let proposedStart = gapStart;
      let proposedEnd = gapEnd;
      const minimumPracticalMinutes=Math.min(240,timeToMinutes(availability.end)-timeToMinutes(availability.start));
      const expanded = expandPracticalSegment(
        { start_time:minutesToTime(gapStart), end_time:minutesToTime(gapEnd) },
        availability,
        remaining - (gapEnd - gapStart),
        blocked,
        minimumPracticalMinutes,
      );
      if (expanded.expanded) {
        const expandedStart = minutesToTime(expanded.start);
        const expandedEnd = minutesToTime(expanded.end);
        if (!wouldExceedClassMax({ employeeId:employee.id, classId:classItem.id, date:gap.date, start:expandedStart, end:expandedEnd })) {
          proposedStart = expanded.start;
          proposedEnd = expanded.end;
        }
      }
      const start = minutesToTime(proposedStart);
      const end = minutesToTime(proposedEnd);
      if (wouldExceedClassMax({ employeeId:employee.id, classId:classItem.id, date:gap.date, start, end })) continue;
      const base = employeeBaseScore({ employee, classItem, date:gap.date, constraints, availability, previousShifts });
      const reasons = [...base.reasons];
      reasons.push(needsLeader ? 'נדרש/ת אחראי/ת כיתה בטווח הזה' : 'קיים חוסר בכוח אדם בטווח הזה');
      const priorEnd=priorForEmployee.length?Math.max(...priorForEmployee.map((item)=>timeToMinutes(item.gap_end_time||item.end_time))):null;
      const gapDistance=priorEnd==null?Infinity:Math.max(0,gapStart-priorEnd);
      const continuityBonus=priorForEmployee.length ? Math.max(90,220-Math.floor(gapDistance/15)*4) : 0;
      if(continuityBonus)reasons.push('עדיפות לפתרון רציף עם אותה עובדת במקום הגעה נוספת');
      reasons.push(`הצעה ידנית בלבד: ${start}–${end}`);
      const utility = base.score + (needsLeader ? 75 : 45) + continuityBonus - Math.max(0, proposedEnd - proposedStart - (gapEnd - gapStart)) / 60;
      options.push({ employee, start, end, gapStart, gapEnd, availability, baseScore:base.score, reasons, utility });
    }
    const chosen = stableSortCandidates(options)[0];
    if (!chosen) continue;
    const key = [gap.date,gap.class_id,chosen.employee.id,chosen.start,chosen.end].join('|');
    if (suggestionKeys.has(key)) continue;
    suggestionKeys.add(key);
    rawReviewSuggestions.push({
      id:key,
      shift_date:gap.date,
      class_id:gap.class_id,
      employee_id:chosen.employee.id,
      start_time:chosen.start,
      end_time:chosen.end,
      gap_start_time:minutesToTime(chosen.gapStart),
      gap_end_time:minutesToTime(chosen.gapEnd),
      availability_start:chosen.availability.start,
      availability_end:chosen.availability.end,
      shift_role:roleForEmployee(chosen.employee),
      score:scoreTo100(chosen.baseScore),
      gap_code:gap.code,
      reasons:chosen.reasons,
      explanation:needsLeader
        ? `מומלץ להוסיף את ${chosen.employee.full_name} כדי להשלים אחראי/ת כיתה בשעות ${chosen.start}–${chosen.end}.`
        : `מומלץ להוסיף את ${chosen.employee.full_name} כדי להשלים את החוסר בשעות ${chosen.start}–${chosen.end}.`,
    });
  }

  function bridgeClassForReview(employee,date,start,end,preferredIds=[]){
    if(end<=start)return null;
    const blocked=finalRows.filter((row)=>row.employee_id===employee.id&&row.shift_date===date);
    if(blocked.some((row)=>timeToMinutes(row.start_time)<end&&timeToMinutes(row.end_time)>start))return null;
    const ids=[...new Set([...preferredIds,employee.primary_class_id,...activeClasses.map((row)=>row.id)].filter(Boolean))];
    const options=ids.map((id)=>{
      const classItem=activeClasses.find((row)=>row.id===id);if(!classItem)return null;
      const eligibility=classEligibility({employee,classItem,date,constraints});if(!eligibility.eligible)return null;
      const startTime=minutesToTime(start),endTime=minutesToTime(end);
      if(wouldExceedClassMax({employeeId:employee.id,classId:id,date,start:startTime,end:endTime}))return null;
      const availability=employeeAvailability({employee,date,patterns,requests,settings});
      const base=employeeBaseScore({employee,classItem,date,constraints,availability,previousShifts});
      const preferredIndex=preferredIds.indexOf(id);
      const utility=base.score+(preferredIndex===0?90:preferredIndex===1?55:0)+(employee.primary_class_id===id?20:0);
      return {id,utility};
    }).filter(Boolean).sort((a,b)=>b.utility-a.utility);
    return options[0]?.id||null;
  }

  function mergeReviewSegments(segments){
    const merged=[];
    for(const segment of segments.sort((a,b)=>timeToMinutes(a.start_time)-timeToMinutes(b.start_time))){
      const previous=merged[merged.length-1];
      if(previous&&previous.class_id===segment.class_id&&timeToMinutes(previous.end_time)===timeToMinutes(segment.start_time)){
  ...
  function makeReviewPlan(items,segments){
    const employee=activeEmployees.find((row)=>row.id===items[0]?.employee_id);
    const merged=mergeReviewSegments(segments);
    const first=merged[0],last=merged[merged.length-1];
    if(!employee||!first||!last)return items;
    const duration=merged.reduce((sum,row)=>sum+timeToMinutes(row.end_time)-timeToMinutes(row.start_time),0);
    const remaining=Math.max(0,maxMinutes(employee)-calculateWeeklyMinutes(finalRows,employee.id));
    if(duration>remaining)return items;
    const classNames=merged.map((segment)=>activeClasses.find((row)=>row.id===segment.class_id)?.name||'כיתה');
    const route=merged.map((segment,index)=>`${segment.start_time}–${segment.end_time} ${classNames[index]}`).join(' ← ');
    const reasons=[`פתרון מרוכז לעובד/ת אחד/ת במקום ${items.length} השלמות נפרדות`,...new Set(items.flatMap((item)=>item.reasons||[]))].slice(0,5);
    return [{
      id:['plan',items[0].shift_date,employee.id,...merged.map((row)=>[row.class_id,row.start_time,row.end_time].join('-'))].join('|'),
      plan:true,
      shift_date:items[0].shift_date,
      class_id:first.class_id,
      employee_id:employee.id,
      start_time:first.start_time,
      end_time:last.end_time,
      shift_role:roleForEmployee(employee),
      score:Math.round(items.reduce((sum,item)=>sum+Number(item.score||0),0)/items.length),
      gap_code:'combined_coverage',
      reasons,
      segments:merged.map((row)=>({...row,shift_date:items[0].shift_date,employee_id:employee.id,shift_role:roleForEmployee(employee)})),
      explanation:`עדיף להביא את ${employee.full_name} לרצף עבודה אחד: ${route}. כך מכסים כמה חוסרים בלי להביא אותה לכמה מקטעים נפרדים ובלי להזמין עובד נוסף שלא לצורך.`,
    }];
  }

  const groupedReview=new Map();
  for(const suggestion of rawReviewSuggestions){
    const key=`${suggestion.shift_date}|${suggestion.employee_id}`;
    if(!groupedReview.has(key))groupedReview.set(key,[]);
    groupedReview.get(key).push(suggestion);
  }
  reviewSuggestions=[];
  for(const items of groupedReview.values()){
    const ordered=[...items].sort((a,b)=>timeToMinutes(a.gap_start_time||a.start_time)-timeToMinutes(b.gap_start_time||b.start_time));
    if(ordered.length===1){reviewSuggestions.push(ordered[0]);continue;}
    const employee=activeEmployees.find((row)=>row.id===ordered[0].employee_id);
    if(!employee){reviewSuggestions.push(...ordered);continue;}
    let cluster=[];let segments=[];let lastEnd=null;let lastClass=null;
    const flush=()=>{if(!cluster.length)return;reviewSuggestions.push(...makeReviewPlan(cluster,segments));cluster=[];segments=[];lastEnd=null;lastClass=null;};
    for(const item of ordered){
      const gapStart=timeToMinutes(item.gap_start_time||item.start_time),gapEnd=timeToMinutes(item.gap_end_time||item.end_time);
      if(lastEnd!==null&&gapStart>lastEnd){
        const bridgeClass=bridgeClassForReview(employee,item.shift_date,lastEnd,gapStart,[lastClass,item.class_id]);
        if(!bridgeClass){flush();}
        else segments.push({class_id:bridgeClass,start_time:minutesToTime(lastEnd),end_time:minutesToTime(gapStart)});
      }
      if(lastEnd!==null&&gapStart<lastEnd){flush();}
      cluster.push(item);
      segments.push({class_id:item.class_id,start_time:minutesToTime(gapStart),end_time:minutesToTime(gapEnd)});
      lastEnd=gapEnd;lastClass=item.class_id;
    }
    flush();
  }

  return {
    weekStart, selectedDates:dates, mode, keptCount: kept.length, generated: cleanGenerated, finalRows,
    validation, daySummaries, employeeHours, excluded, assignmentNotes, coverageGaps, reviewSuggestions,
    metrics: {
      ...
      borrowedCount, mandatoryWorkMissed: mandatoryMissing.length, unresolvedStaffMinutes, reviewSuggestionCount: reviewSuggestions.length,
    },
    signature: crypto.createHash('sha256').update(`${automaticInputSignature(input,dates,mode)}|${shiftSignature(finalRows)}`).digest('hex').slice(0,24),
  };
}

module.exports = {
  generateAutomaticSchedule, employeeAvailability, employeeBaseScore, roleForEmployee,
  scoreTo100, shiftSignature, activeConstraint, absentOn, partialAsNeededIssue, automaticInputSignature,
};
