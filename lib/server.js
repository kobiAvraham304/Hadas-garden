...
async function sendPushNotifications(employeeIds,{title,message='',url='/',tag='hadas',type='info'}={}) {
  ...
  try {
    ...
    webpush.setVapidDetails(config.subject||'https://hadas-garden.vercel.app',config.public_key,config.private_key);
    ...
    return Promise.allSettled(subscriptions.map(async row=>{
      try {
        await webpush.sendNotification({endpoint:row.endpoint,keys:{p256dh:row.p256dh,auth:row.auth}},payload,{TTL:3600,urgency:type==='urgent'?'high':'normal'});
