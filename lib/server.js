...
function getEnv() {
  const required = ['SUPABASE_URL', 'SUPABASE_PUBLISHABLE_KEY', 'SUPABASE_SECRET_KEY', 'SESSION_PEPPER'];
...
async function fetchJson(url, options = {}) {
  try {
    ...
    return { data: null, error: { message: error.message || 'Network error', details: error } };
  }
}
...
class StorageBucket {
  ...
  async createSignedUploadUrl(path) {
    const fullPath = `${encodeURIComponent(this.bucket)}/${encodePath(path)}`;
    const result = await fetchJson(`${getEnv().url}/storage/v1/object/upload/sign/${fullPath}`, {
      method: 'POST', headers: apiHeaders({ 'Content-Type': 'application/json' }), body: '{}',
    });
    if (result.error) return result;
    const relative = result.data?.url;
    if (!relative) return { data: null, error: { message: 'No signed upload URL returned' } };
    const storageBase = `${getEnv().url}/storage/v1`;
    const signedUrl = /^https?:\/\//i.test(relative)
      ? relative
      : `${storageBase}${relative.startsWith('/') ? '' : '/'}${relative}`;
    const token = new URL(signedUrl).searchParams.get('token');
    if (!token) return { data: null, error: { message: 'No upload token returned' } };
    return { data: { signedUrl, path, token }, error: null };
  }
  async createSignedUrl(path, expiresIn, options = {}) {
    const fullPath = `${encodeURIComponent(this.bucket)}/${encodePath(path)}`;
    const result = await fetchJson(`${getEnv().url}/storage/v1/object/sign/${fullPath}`, {
      method: 'POST', headers: apiHeaders({ 'Content-Type': 'application/json' }), body: JSON.stringify({ expiresIn }),
    });
    if (result.error) return result;
    const relative = result.data?.signedURL || result.data?.signedUrl;
    if (!relative) return { data: null, error: { message: 'No signed URL returned' } };
    const storageBase = `${getEnv().url}/storage/v1`;
    const url = /^https?:\/\//i.test(relative)
      ? relative
      : `${storageBase}${relative.startsWith('/') ? '' : '/'}${relative}`;
    const download = options.download ? `&download=${encodeURIComponent(options.download === true ? '' : options.download)}` : '';
    return { data: { signedUrl: `${url}${download}` }, error: null };
  }
  async remove(paths) {
    return fetchJson(`${getEnv().url}/storage/v1/object/${encodeURIComponent(this.bucket)}`, {
      method: 'DELETE', headers: apiHeaders({ 'Content-Type': 'application/json' }), body: JSON.stringify({ prefixes: paths }),
    });
  }
}
...
async function getSession(req, { optional = false } = {}) {
  ...
  const user = assertDb(await db().from('hadas_users').select('*').eq('id', session.user_id).maybeSingle(), 'המשתמשת לא נמצאה');
  if (!user || !user.active) throw httpError(403, 'המשתמשת אינה פעילה במערכת');
  const employee = assertDb(await db().from('hadas_employees').select('*').eq('id', user.employee_id).maybeSingle(), 'כרטיס העובדת לא נמצא');
  if (!employee || !employee.active) throw httpError(403, 'העובדת אינה פעילה במערכת');
...
async function revokeUserSessions(userId) { assertDb(await db().from('hadas_sessions').update({ revoked_at: new Date().toISOString() }).eq('user_id', userId).is('revoked_at', 'null'), 'לא ניתן היה לנתק את המשתמשת'); }
...
module.exports = { SESSION_COOKIE, getEnv, db, parseBody, normalizePhone, displayPhone, httpError, assertDb, send, handleError, parseCookies, setSessionCookie, clearSessionCookie, randomToken, sha256, safeEqual, clientIp, verifyOrigin, createSession, getSession, requireSession, isManager, revokeUserSessions, emitEvent, audit, hashPassword, verifyPassword, publicProfile };
