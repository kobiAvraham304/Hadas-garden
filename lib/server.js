...
let client;
let pushConfigCache = { value: null, expiresAt: 0 };
...
async function fetchJson(url, options = {}) {
  ...
  // One deadline covers all retries, so a failing read cannot consume 3 timeouts.
  const deadline = Date.now() + timeoutMs;
  const method = String(options.method || 'GET').toUpperCase();
  // Reads are safe to retry when Supabase's gateway has a short transient outage.
  // Writes are deliberately never retried here to avoid duplicate mutations.
  const canRetry = !options.signal && (method === 'GET' || method === 'HEAD');
  const maxAttempts = canRetry ? 3 : 1;
  const transientStatuses = new Set([502, 503, 504]);

  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      const requestOptions = options.signal ? options : { ...options, signal:AbortSignal.timeout(Math.max(1, deadline - Date.now())) };
      ...
      if (canRetry && transientStatuses.has(response.status) && attempt < maxAttempts && Date.now() + attempt * 220 < deadline) {
      ...
      if (canRetry && !timedOut && attempt < maxAttempts && Date.now() + attempt * 220 < deadline) {
        await new Promise((resolve) => setTimeout(resolve, attempt * 220));
        continue;
      }
  ...
  return { data:null, error:{ message:'החיבור ל־Supabase לא זמין כרגע' } };
}
...
function handleError(res, error) {
  ...
  // Authentication, validation and conflict responses are expected application
  // outcomes. Logging them as runtime errors hid real production failures.
  if (status >= 500) {
    console.error('Hadas API request failed', {
      status,
...
async function requireSession(req, { manager = false, csrf = true } = {}) {
  ...
  const caller = await (req.hadasSessionContext ||= getSession(req));
...
function canManageDailyOperations(caller) {
  const title = String(caller?.employee?.job_title || '').trim();
  return isManager(caller) || isTeacher(caller) || title === 'מנהלת מעון';
}
function scheduleScope(caller) {
  const title = String(caller?.employee?.job_title || '');
  if (isManager(caller) || isTeacher(caller) || ['אחות','מנהלת מעון','מזכירה'].includes(title)) return 'full';
  if (title === 'סייעת מובילה' && caller?.employee?.primary_class_id) return 'class';
  return 'personal';
}
function canViewFullSchedule(caller) { return scheduleScope(caller) === 'full'; }
function canViewClassSchedule(caller) { return scheduleScope(caller) === 'class'; }
...
async function loadPushConfig() {
  if (pushConfigCache.value && pushConfigCache.expiresAt > Date.now()) return pushConfigCache.value;
  const result = await db().from('hadas_push_config').select('public_key,private_key,subject').eq('id', 1).maybeSingle();
  if (result.error || !result.data?.public_key || !result.data?.private_key) return null;
  pushConfigCache = { value:result.data, expiresAt:Date.now() + 10 * 60 * 1000 };
  return result.data;
}
function inferredPushUrl(entityType, entityId) {
  if (entityType === 'schedule' && entityId) return `/?push=schedule&week_start=${encodeURIComponent(String(entityId))}`;
  if (entityType === 'request' && entityId) return `/?push=request&id=${encodeURIComponent(String(entityId))}`;
  if (entityType === 'announcement' && entityId) return `/?push=announcement&id=${encodeURIComponent(String(entityId))}`;
  return '/?push=notifications';
}
function automaticPushForNotification(type, title) {
  const value=String(title||'');
  if(type==='schedule'&&value==='פורסם שיבוץ שבועי חדש')return true;
  if(type==='request'&&['הבקשה שלך אושרה','הבקשה שלך נדחתה','ביטול החופשה אושר','ביטול החופשה לא אושר'].includes(value))return true;
  if(type==='swap'&&['בקשת ההחלפה אושרה והוזרמה','בקשת ההחלפה נדחתה'].includes(value))return true;
  return false;
}
...
function vapidAuthorization(endpoint,config){
  const publicRaw=Buffer.from(config.public_key,'base64url');
  if(publicRaw.length!==65||publicRaw[0]!==4)throw new Error('Invalid VAPID public key');
  const x=publicRaw.subarray(1,33).toString('base64url'),y=publicRaw.subarray(33,65).toString('base64url');
  const key=crypto.createPrivateKey({key:{kty:'EC',crv:'P-256',x,y,d:config.private_key},format:'jwk'});
  const header=b64url(JSON.stringify({typ:'JWT',alg:'ES256'}));
  const payload=b64url(JSON.stringify({aud:new URL(endpoint).origin,exp:Math.floor(Date.now()/1000)+43200,sub:config.subject||'https://hadas-garden.vercel.app'}));
  const input=`${header}.${payload}`;
  const signature=crypto.sign('sha256',Buffer.from(input),{key,dsaEncoding:'ieee-p1363'}).toString('base64url');
  return `vapid t=${input}.${signature}, k=${config.public_key}`;
}
function encryptWebPushPayload(subscription,payload){
  const clientPublic=Buffer.from(subscription.p256dh,'base64url'),authSecret=Buffer.from(subscription.auth,'base64url');
  if(clientPublic.length!==65||clientPublic[0]!==4||!authSecret.length)throw new Error('Invalid push subscription key');
  const ecdh=crypto.createECDH('prime256v1');ecdh.generateKeys();
  const serverPublic=ecdh.getPublicKey(),shared=ecdh.computeSecret(clientPublic);
  ...
  const ikm=hkdfExpand(prkKey,keyInfo,32),salt=crypto.randomBytes(16),prk=hkdfExtract(salt,ikm);
  const cek=hkdfExpand(prk,Buffer.from('Content-Encoding: aes128gcm\0'),16),nonce=hkdfExpand(prk,Buffer.from('Content-Encoding: nonce\0'),12);
  const plain=Buffer.concat([Buffer.from(payload),Buffer.from([2])]);
  const cipher=crypto.createCipheriv('aes-128-gcm',cek,nonce),encrypted=Buffer.concat([cipher.update(plain),cipher.final(),cipher.getAuthTag()]);
  const header=Buffer.alloc(21);salt.copy(header,0);header.writeUInt32BE(4096,16);header[20]=serverPublic.length;
  return Buffer.concat([header,serverPublic,encrypted]);
}
async function sendPushNotifications(employeeIds,{title,message='',url='/',tag='hadas',type='info'}={}) {
  const ids=[...new Set((employeeIds||[]).map(String).filter(Boolean))];
  if(!ids.length||!title)return [];
  try {
    const config=await loadPushConfig(); if(!config)return [];
    const result=await db().from('hadas_push_subscriptions').select('id,employee_id,endpoint,p256dh,auth,failure_count').in('employee_id',ids).eq('active',true);
    if(result.error)return [];
    const subscriptions=result.data||[]; if(!subscriptions.length)return [];
    const payload=JSON.stringify({title:String(title).slice(0,180),body:String(message||'').slice(0,280),url:String(url||'/').startsWith('/')?String(url||'/'):'/',tag:String(tag||'hadas').slice(0,80),type:String(type||'info').slice(0,40),icon:'/favicon-192x192.png?v=0171',badge:'/favicon-32x32.png'});
    return Promise.allSettled(subscriptions.map(async row=>{
      try {
        const body=encryptWebPushPayload(row,payload);
        const response=await fetch(row.endpoint,{method:'POST',headers:{TTL:'3600','Content-Encoding':'aes128gcm','Content-Type':'application/octet-stream',Authorization:vapidAuthorization(row.endpoint,config)},body});
        if(!response.ok){const error=new Error('Push service rejected notification');error.statusCode=response.status;throw error;}
        await db().from('hadas_push_subscriptions').update({failure_count:0,last_success_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq('id',row.id);
        return {id:row.id,ok:true};
      } catch(error) {
        const status=Number(error?.statusCode||error?.status||0);
        if([404,410].includes(status))await db().from('hadas_push_subscriptions').delete().eq('id',row.id);
        else {const failures=Number(row.failure_count||0)+1;await db().from('hadas_push_subscriptions').update({failure_count:failures,active:failures<3,updated_at:new Date().toISOString()}).eq('id',row.id);}
        console.warn('Push delivery failed',{status,employeeId:row.employee_id});return {id:row.id,ok:false,status};
      }
    }));
  } catch(error){console.error('Push delivery unavailable',error);return [];}
}
async function notifyEmployees(employeeIds, { type='info', title, message='', entityType=null, entityId=null, actionRequired=false, push=false, pushUrl=null, pushTag=null } = {}) {
  const ids = [...new Set((employeeIds || []).map(String).filter(Boolean))];
  if (!ids.length || !title) return [];
  ...
  if(push||automaticPushForNotification(type,title)) await sendPushNotifications(ids,{title,message,url:pushUrl||inferredPushUrl(entityType,entityId),tag:pushTag||`${entityType||type||'hadas'}-${entityId||Date.now()}`,type});
  return result.data || [];
}
