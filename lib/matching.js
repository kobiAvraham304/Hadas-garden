const { overlaps, timeToMinutes, calculateWeeklyMinutes, closingTimeForDate, requiredStaffAt } = require('./schedule');
...
function sourceClassCanRelease(context, sourceClassId, employeeId, date, start, end) {
  ...
  for (let point = rangeStart; point < rangeEnd; point += slotMinutes) {
    ...
    const expected = requiredStaffAt(context.settings || {}, date, point);
