const { overlaps, timeToMinutes, calculateWeeklyMinutes, closingTimeForDate } = require('./schedule');
...
function sourceClassCanRelease(context, sourceClassId, employeeId, date, start, end) {
  ...
  const slotMinutes = Math.max(15, Number(context.settings?.validation_slot_minutes || 30));
  const closingWindow = Math.max(0, Number(context.settings?.closing_window_minutes || 30));
  ...
  for (let point = rangeStart; point < rangeEnd; point += slotMinutes) {
    const slotStart = `${String(Math.floor(point / 60)).padStart(2, '0')}:${String(point % 60).padStart(2, '0')}`;
    const slotEndMinutes = Math.min(point + slotMinutes, rangeEnd);
    ...
    const expected = closingWindow > 0 && point >= closing - closingWindow
      ? Number(context.settings?.closing_required_staff || 3)
      : Number(context.settings?.required_staff || 4);
    if (count < expected) return false;
    if (context.settings?.require_leader !== false && !remaining.some((row) => ['teacher', 'lead'].includes(row.shift_role))) return false;
  }
  return true;
}
...
function rankCandidates(context) {
  ...
  for (const employee of employees) {
    ...
    if (constraint?.constraint_type === 'forbidden') { reject('קיים איסור שיבוץ בכיתה'); continue; }
