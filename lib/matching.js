...
function containsRange(shift, start, end) {
  return timeToMinutes(shift?.start_time) <= timeToMinutes(start) && timeToMinutes(shift?.end_time) >= timeToMinutes(end);
}
...
function scoreCandidate({ employee, targetClassId, neededRole, pattern, constraint, weeklyMinutes, requestedMinutes, candidateType, sourceShift, availability }) {
  ...
  if (pattern?.day_type === 'as_needed') {
    if (employee.assignment_mode === 'substitute') {
      score += 12;
      reasons.push('מוגדר/ת כממלא/ת מקום וזמין/ה לפי צורך');
    } else if (employee.assignment_mode === 'rotation') {
      score += 2;
      reasons.push('זמין/ה לפי צורך וברוטציה');
    } else {
      score -= 14;
      reasons.push('זמין/ה לפי צורך');
      cautions.push('עדיפות אחרי עובד/ת ביום עבודה קבוע');
    }
  }
  if (pattern?.day_type === 'avoid') { score -= 34; reasons.push('עדיף להימנע ביום זה'); cautions.push('עדיפות נמוכה — לבחור רק אם אין חלופה'); }
...
function rankCandidates(context) {
  const {
    ...
    date, classId, start, end, neededRole = 'staff', excludedEmployeeId = null, excludeShiftId = null, allowPartialTransfer = false,
  } = context;
  ...
  for (const employee of employees) {
    ...
    if (overlapping.length === 1) {
      ...
      const sourceClassName = classes.find((item) => item.id === sourceShift.class_id)?.name || 'כיתה אחרת';
      const sourceRange = `${short(sourceShift.start_time)}–${short(sourceShift.end_time)}`;
      if (allowPartialTransfer) {
        if (!containsRange(sourceShift, start, end)) {
          reject(`משובץ/ת ב־${sourceClassName} · ${sourceRange}, אך השיבוץ אינו מכסה את כל טווח החוסר`);
          continue;
        }
        transferMode = exactRange(sourceShift, start, end) ? 'full' : 'partial';
      } else {
        if (!exactRange(sourceShift, start, end)) {
          reject(`משובץ/ת ב־${sourceClassName} · ${sourceRange}. בעריכת שיבוץ ניתן להעביר רק שיבוץ עם טווח שעות זהה`);
          continue;
        }
        transferMode = 'full';
      }
      if (!sourceClassCanRelease(mergedContext, sourceShift.class_id, employee.id, date, start, end)) {
        reject(`משובץ/ת ב־${sourceClassName} · ${sourceRange}, אך העברה תפגע בתקינת כיתת המקור`);
        continue;
      }
    ...
    candidates.push({
      ...
      source_start_time: sourceShift ? short(sourceShift.start_time) : null,
      source_end_time: sourceShift ? short(sourceShift.end_time) : null,
