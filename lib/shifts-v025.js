const legacyHandler = require('../handlers/shifts');
const {
  requireSession, parseBody, db, assertDb, send, handleError, httpError,
} = require('./server');
const { timeToMinutes, closingTimeForDate } = require('./schedule');
const { employeeCanLead } = require('../handlers/daily-operations');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (clearAction) {
      const rawWeekStart = String(body.week_start || '');
      if (!/^\d{4}-\d{2}-\d{2}$/.test(rawWeekStart)) throw httpError(400, 'תאריך השבוע שנבחר אינו תקין');
      const weekStart = getSunday(rawWeekStart);
      if (weekStart !== rawWeekStart) throw httpError(400, 'תאריך השבוע שנבחר אינו תקין');
