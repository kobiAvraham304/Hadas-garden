...
function validationIssueKey(issue = {}) {
  ...
  const stable = { code };
  // Automatic-schedule approvals must survive the post-apply coverage collapse,
  // where the exact time range/count/message can legitimately change.
...
async function generalDayOffDates(weekStart) {
  const weekEnd = addDays(weekStart, 5);
  const rows = assertDb(await db().from('hadas_calendar_events')
    .select('event_date,is_general_day_off')
    .eq('is_general_day_off', true)
    .gte('event_date', weekStart)
    .lte('event_date', weekEnd), 'לא ניתן לטעון חופשות כלליות') || [];
  return new Set(rows.map((row) => String(row.event_date || '')));
}
function withoutGeneralDayIssues(validation, offDates) {
  const keep = (item) => !item?.date || !offDates.has(String(item.date));
  return {
    ...(validation || {}),
    errors:(validation?.errors || []).filter(keep),
    warnings:(validation?.warnings || []).filter(keep),
  };
}

async function rawWeekValidation(weekStart) {
  const weekEnd = addDays(weekStart, 5);
  const [shiftsR, classesR, employeesR, settingsR, constraintsR, patternsR, requestsR, offDates] = await Promise.all([
    ...
    generalDayOffDates(weekStart),
  ]);
  const shifts = assertDb(shiftsR, 'לא ניתן לטעון שיבוצים') || [];
  const validation = withoutGeneralDayIssues(validateWeekUnapproved({
    shifts,
...
async function approvedKeys(weekStart) {
  ...
  for (const row of rows) {
    if (row.issue_key) keys.add(String(row.issue_key));
    if (row.issue_snapshot) keys.add(validationIssueKey(row.issue_snapshot));
  }
  return { rows, keys };
}
...
module.exports = async function shiftsV030(req, res) {
  try {
    ...
    if (req.method === 'POST' && ['validate', 'publish_preview'].includes(action)) {
      ...
      const weekStart = getSunday(String(body.week_start || israelDateISO()));
      const [approvals, offDates] = await Promise.all([approvedKeys(weekStart), generalDayOffDates(weekStart)]);
      const live = withoutGeneralDayIssues({ errors:payload.errors || [], warnings:payload.warnings || [] }, offDates);
      const rawErrors = live.errors.map(decorateIssue);
      payload.errors = rawErrors.filter((item) => !approvals.keys.has(item.approval_key));
      payload.warnings = live.warnings.map(decorateIssue);
