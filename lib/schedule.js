...
function validateWeek({ shifts, classes, employees, settings, weekStart, constraints = [], weeklyPatterns = [] }) {
      ...
      for (let j = i + 1; j < employeeShifts.length; j++) {
        ...
        if (first.shift_date === second.shift_date && overlaps(first.start_time, first.end_time, second.start_time, second.end_time)) {
          errors.push({
            ...
            message: `${employee.full_name} משובץ בשעות חופפות`,
          });
        }
      }
    }
  ...
  for (const shift of shifts) {
    ...
    if (forbidden) {
      ...
      errors.push({
        ...
        message: `לא ניתן לשבץ את ${employee?.full_name || 'העובד'} בכיתת ${classItem?.name || ''}`,
      });
    }
    ...
    if (pattern?.day_type === 'day_off') {
      errors.push({
        ...
        employee_id: shift.employee_id,
        class_id: shift.class_id,
        message: `${employee?.full_name || 'העובד'} משובץ ביום חופשי קבוע`,
      });
      ...
      warnings.push({
        code: 'outside_fixed_hours',
        date: shift.shift_date,
        employee_id: shift.employee_id,
        class_id: shift.class_id,
        message: `${employee?.full_name || 'העובד'} שובץ מחוץ לשעות הקבועות ${String(pattern.start_time).slice(0,5)}–${String(pattern.end_time).slice(0,5)}`,
      });
    }
  }
      ...
      for (let slot = opening; slot < closing; slot += slotMinutes) {
        ...
        if (people.size < expected) {
          errors.push({
            ...
            message: `${classItem.name}: ${people.size} אנשי צוות בשעה ${minutesToTime(slot)} במקום ${expected}`,
          });
        }
        ...
        if (!hasLeader) {
          errors.push({
            ...
            time: minutesToTime(slot),
            end_time: minutesToTime(slotEnd),
            message: `${classItem.name}: חסר גננת/גנן או מוביל/ה בשעה ${minutesToTime(slot)}`,
          });
        }
      }
    }
  ...
  for (const employee of activeEmployees) {
    ...
    if (maximum != null && weeklyMinutes > maximum) {
      ...
      errors.push({
        ...
        employee_id: employee.id,
        message: `${employee.full_name}: שובץ ${actual} שעות ועבר את המקסימום השבועי ${maxHours}`,
      });
    }
  ...
  return {
    errors: collapseCoverageIssues(errors, activeClasses),
