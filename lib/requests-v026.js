...
function canPreApprove(body = {}) {
  if (String(body.request_type || '') === 'swap') return false;
  return truthy(body.pre_approved) || truthy(body.apply_now);
}
...
module.exports = async function requestsV026(req, res) {
  try {
    ...
    const requestedPreApproval = truthy(body.pre_approved) || truthy(body.apply_now);
    const explicitlyApproved = canPreApprove(body);
    ...
    // החלפת שיבוץ נשארת pending גם אם סומן "מאושר מראש" — אישור העובד היעד הוא כלל שאסור לעקוף.
    ...
    const swapGuard = updated.request_type === 'swap';
    await notifyManagers({
      type: 'request',
      title: swapGuard ? 'בקשת החלפה ממתינה לאישור העובד' : 'בקשה חדשה לעובד ממתינה לאישור',
      message: swapGuard
        ? `הוזנה בקשת החלפת שיבוץ עבור עובד (${requestRange(updated)}). גם בקשה שסומנה מאושרת מראש מחייבת קודם את אישור העובד היעד.`
        : `הוזנה בקשת ${requestLabel(updated.request_type)} עבור עובד (${requestRange(updated)}). הבקשה לא סומנה כמאושרת מראש.`,
      entityType: 'request',
      entityId: updated.id,
      actionRequired: true,
    }, updated.created_by || null);
    await audit(updated.created_by || null, 'manager_request_pending', 'request', updated.id, {
      pre_approved: false,
      requested_pre_approval: requestedPreApproval,
      swap_target_consent_guard: swapGuard,
    });
    await emitEvent('requests');

    payload.request = updated;
    payload.pre_approved = false;
    payload.pre_approval_ignored_for_swap = swapGuard && requestedPreApproval;
...
module.exports.canPreApprove = canPreApprove;
