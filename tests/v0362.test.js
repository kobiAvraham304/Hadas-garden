...
test('0.36 shift editor explains real unavailability and assignment conflicts', () => {
  ...
  assert.match(app, /state\.shifts \|\| \[\]/);
  assert.match(app, /overlaps\(start, end, shift\.start_time, shift\.end_time\)/);
...
test('0.36 hf8 cache chain delivers all changed request and shift layers', () => {
  ...
  assert.match(index, /app\.js\?v=0360hf8/);
  assert.match(index, /patch-v025\.js\?v=0360hf8/);
  assert.match(entry, /patch-v026\.js\?v=0321hf8/);
  assert.match(entry, /patch-v033\.js\?v=0333hf8/);
  assert.match(entry, /patch-v0342\.js\?v=0360hf8/);
