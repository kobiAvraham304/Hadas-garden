...
test('version, security headers and health route are consistent', () => {
  ...
  assert.equal(pkg.version,'0.7.0'); assert.equal(Object.hasOwn(pkg,'engines'),false);
...
test('0.7 mobile weekly schedule uses six readable accordion day cards instead of the wide table', () => {
  const app=read('app.js'); const css=read('styles.css');
