...
test('version, security headers and health route are consistent', () => {
  ...
  assert.equal(pkg.version,'0.12.0'); assert.equal(Object.hasOwn(pkg,'engines'),false);
...
test('initial accounts and schema version are present in clean installer', () => {
  ...
  assert.match(schema,/v_initial_hash/); assert.match(schema,/'0\.12\.0'/);
