...
test('unified router resolves the rewritten route query', () => {
...
test('unified router returns JSON 404 for an unknown route', async () => {
  const router=require('../api/index');
  const res=responseMock();
  await router({method:'GET',query:{route:'not-real'},url:'/api/index?route=not-real'},res);
