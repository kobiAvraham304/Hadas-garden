...
test('0.36 daily coverage renders the modern shift-editor matching language', () => {
  ...
  assert.match(app, /candidate-type-badge/);
  assert.match(app, /daily-modern-candidate/);
  assert.match(app, /אותו מנגנון התאמה של עריכת שיבוץ/);
  assert.match(app, /function dailyRejectedReasonHtml/);
  assert.match(app, /למה עובדים אחרים לא זמינים/);
  assert.match(app, /candidate-cautions/);
  assert.match(app, /ביצוע מעבר בטוח/);
