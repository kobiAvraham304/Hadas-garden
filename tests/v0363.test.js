...
test('0.36 hf11 cache chain exposes the daily coverage update', () => {
  ...
  assert.match(index, /app\.js\?v=0380/);
  assert.match(index, /patch-v025\.js\?v=0380/);
...
test('0.38.0 mobile schedule controls remain user-controlled and focus uses visible targets', () => {
...
test('0.38.0 validation toggle exposes a rotating open-state chevron', () => {
  const core = read('patch-v032-core.js');
...
test('0.38.0 mobile compact week can open the existing full horizontal schedule and return', () => {
  const v033 = read('patch-v033.js');
...
test('0.38.0 full horizontal mobile week overrides the legacy hidden-table rule', () => {
  const v033 = read('patch-v033.js');
...
test('0.38.0 primes the wide mobile week before swapping views to avoid Safari blank paint', () => {
  const v033 = read('patch-v033.js');
...
test('0.38.0 adds lightweight motion and respects reduced-motion preferences', () => {
  const v033 = read('patch-v033.js');
