...
test('0.30 release layers remain available under the current release', () => {
  assert.equal(JSON.parse(read('package.json')).version, '0.36.3');
...
test('0.30 runtime stays intact while physical entrypoints advance to v0.34', () => {
  ...
  assert.match(entry, /const VERSION = '0\.36\.3'/);
  assert.match(entry, /V026 = '\/patch-v026\.js\?v=0321hf12'/);
  assert.match(entry, /V033 = '\/patch-v033\.js\?v=0333hf9'/);
  ...
  assert.match(cssEntry, /patch-v034\.css\?v=0350/);
