...
module.exports = async function handler(req,res) {
  try {
    ...
    for (const employee of employees) {
      ...
      let score = 0;
      const reasons = [];
      if (!dayShifts.length) { score += 40; reasons.push('אינה משובצת באותו יום'); }
      ...
      if (neededRole === 'teacher') {
        if (/גננת/.test(title)) { score += 35; reasons.push('גננת'); }
        else if (employee.can_lead) { score += 18; reasons.push('יכולה לשמש מובילה'); }
        else { score -= 30; reasons.push('אינה מוגדרת כגננת או מובילה'); }
      ...
      if (neededRole === 'teacher' && !/גננת/.test(title)) suggestedRole = employee.can_lead ? 'lead' : 'staff';
      candidates.push({
