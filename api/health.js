...
module.exports = async function handler(req, res) {
  ...
  if (!missing.length) {
    try {
      ...
      checks.schema = meta.error
        ? { ok:false, version:null, error:'יש להריץ את קובץ ה-SQL של גרסה 0.4.1' }
        : { ok:Boolean(meta.data && meta.data.schema_version === '0.4.1'), version:meta.data?.schema_version || null };
  ...
  return send(res, ready ? 200 : 503, { ok:ready, version:'0.4.1', checks });
};
